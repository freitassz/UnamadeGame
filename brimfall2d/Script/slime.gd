...
func _on_attack_area_entered(body: CharacterBody2D) -> void:
	# Se o player entrou na área de ataque e o inimigo está perseguindo E pode atacar,
	# ele se compromete com o ataque.
...
func _on_attack_area_exited(body: CharacterBody2D) -> void:
	# Quando o jogador sai da área de ataque, o inimigo não retorna imediatamente a CHASING.
	# Ele primeiro completa o ataque e o cooldown.
	# A transição de volta para CHASING (ou IDLE) agora é tratada APENAS no final de POST_ATTACK_COOLDOWN
	# ou KNOCKBACK.
	# Esta função pode ser simplificada ou até mesmo removida, pois a lógica de transição foi movida.
	# No entanto, mantê-la para futuros debugs ou comportamentos específicos de saída pode ser útil.
	
	# Por enquanto, não faremos nada aqui para forçar o comprometimento.
	pass # Não faz nada aqui para permitir o comprometimento do ataque.
